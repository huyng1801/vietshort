...
import VideoInteractionsTable from '@/components/social/video-interactions/VideoInteractionsTable';
import VideoInteractionsFilters from '@/components/social/video-interactions/VideoInteractionsFilters';
...
export default function VideoInteractionsPage() {
  ...
  const fetchVideos = useCallback(async () => {
    setLoading(true);
    try {
      const res = await adminAPI.getVideoInteractions({
        page: params.page,
        limit: params.limit,
      ...
      message.error('Không thể tải dữ liệu tương tác video');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">
        Tương tác Video (Sưu tầm & Yêu thích)
      </h1>

      <VideoInteractionsFilters
        search={search}
