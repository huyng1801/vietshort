...
export class GoogleOAuthStrategy extends PassportStrategy(GoogleStrategy, 'google') {
  constructor(
    ...
    super({
      ...
      callbackURL: configService.get('oauth.google.callbackUrl') || '/api/v1/auth/google/callback',
      scope: ['email', 'profile'],
    });
  }
...
export class FacebookOAuthStrategy extends PassportStrategy(FacebookStrategy, 'facebook') {
  constructor(
    ...
    super({
      ...
      callbackURL: configService.get('oauth.facebook.callbackUrl') || '/api/v1/auth/facebook/callback',
...
export class AppleOAuthStrategy extends PassportStrategy(AppleStrategy, 'apple') {
  constructor(
    ...
    super({
      clientID: configService.get('oauth.apple.clientId'), // Service ID
      ...
      callbackURL: configService.get('oauth.apple.callbackUrl') || '/api/v1/auth/apple/callback',
      scope: ['email', 'name'],
...
export class TikTokOAuthService {
  ...
  getAuthorizationUrl(state: string): string {
    ...
    const redirectUri = this.configService.get('TIKTOK_REDIRECT_URI') || '/api/v1/auth/tiktok/callback';
  ...
  async handleCallback(code: string) {
    ...
    const redirectUri = this.configService.get('TIKTOK_REDIRECT_URI');
