...
import { Bell, Gift, X, Zap, ChevronRight, Crown, Film, Coins, Flame } from 'lucide-react';
  ...
  return (
    <div className="relative bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/20 rounded-lg sm:rounded-xl p-3 sm:p-4 mb-3 sm:mb-4">
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 lg:gap-3">
        <div className="w-8 h-8 sm:w-9 sm:h-9 lg:w-10 lg:h-10 bg-blue-500/20 rounded-full flex items-center justify-center flex-shrink-0">
...
export function StreakReminder({ currentStreak }: StreakReminderProps) {
  ...
  const messages: { min: number; max: number; text: string; sub: string; color: keyof typeof colorMap }[] = [
    ...
    { min: 6, max: 7, text: 'Sắp đạt đỉnh!', sub: 'Chỉ còn 1 ngày nữa là nhận 50 Gold bonus!', color: 'red' },
  ];
  ...
  return (
    <div className={cn(
      ...
      <div className="text-xl sm:text-2xl"><Flame className={cn('w-5 h-5 sm:w-6 sm:h-6', colors.text)} /></div>
      <div>
        <p className={cn('text-xs sm:text-sm font-bold', colors.text)}>{msg.text}</p>
...
export function BehaviorOffer() {
  ...
  return (
    <div className={cn('relative rounded-lg sm:rounded-xl border p-3 sm:p-4 bg-gradient-to-r', offer.bg, offer.border)}>
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 lg:gap-3">
        <div className="w-8 h-8 sm:w-9 sm:h-9 lg:w-10 lg:h-10 bg-white/5 rounded-full flex items-center justify-center flex-shrink-0">
          {offer.icon}
        </div>
        <div className="flex-1 min-w-0">
...
export function CountdownPressure({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 lg:gap-3">
        <div className={cn(
          'w-10 h-10 sm:w-11 sm:h-11 lg:w-12 lg:h-12 rounded-lg sm:rounded-xl flex items-center justify-center font-mono text-xs sm:text-sm font-bold flex-shrink-0 border',
          isUrgent
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5">
            <Zap className={cn('w-3 h-3 sm:w-3.5 sm:h-3.5', isUrgent ? 'text-red-400' : 'text-amber-400')} />
