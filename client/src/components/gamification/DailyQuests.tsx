...
import { CheckCircle, Circle, Coins, Zap, ChevronRight, Play, Tv, PartyPopper } from 'lucide-react';
...
function QuestCard({ task }: QuestCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'w-10 h-10 sm:w-12 h-12 lg:w-14 lg:h-14 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0',
      ...
      <div className="flex-shrink-0">
        {task.completed ? (
          <CheckCircle className="w-5 h-5 sm:w-6 h-6 text-green-400" />
        ) : (
          <Circle className="w-5 h-5 sm:w-6 h-6 text-gray-600" />
        )}
      </div>
    </div>
  );
  ...
  if (error || tasks.length === 0) {
    return (
      <div className="text-center py-6 sm:py-8 text-gray-500">
        <Tv className="w-8 h-8 sm:w-10 h-10 mx-auto mb-2 sm:mb-3 opacity-50" />
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-3 sm:mb-4 gap-2">
        <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-2.5">
          <Zap className="w-4 h-4 sm:w-5 h-5 text-amber-400" />
